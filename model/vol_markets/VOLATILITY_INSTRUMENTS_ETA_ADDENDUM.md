...
> paper's curvature index `k` is written `κ_φ`; his investor private-use premium `α` is written <!-- notation-map -->
...
**The "two antitone objectives, opposite corners, therefore an interior peak" reading is FALSE and is not made here.** A nonnegative weighting \(w_1(-\mathrm{arbLoss}) + w_2\,\mathrm{surplus}\) has, on each branch, a derivative whose SIGN IS CONSTANT on that branch and depends only on the weights — on \([0,\kappa_{\varphi,S}]\) it is \(\tfrac{w_1\varpi_A - w_2}{2}\cdot\tfrac{1+\phi}{(1-\kappa_{\varphi})^2}\). Such a combination is piecewise monotone and peaks at a branch point only by accident of the weights. Scalarization does not manufacture an interior optimum, which is the same lesson Phase 11's T22 already recorded.
